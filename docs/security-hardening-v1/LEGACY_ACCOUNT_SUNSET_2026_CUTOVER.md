...
8. Only after all preflight/rollback records exist, deploy `my-gym-log-retention` on Workers Free from `retention/wrangler.jsonc`. The hourly UTC `:15` trigger has no deletion before `2026-12-31T16:15:00Z`; verify Cron, Worker source hash, Free plan, separate service account and Secrets binding name. An HTTP request cannot invoke deletion. If any target has Google linked, Worker must report NO DELETE regardless of Firestore state.
