...
const makeMovementSavePayload = (movement) => {
    ...
    const movementWithoutDocId = { ...(movement || {}) };
    delete movementWithoutDocId.id;
...
const ProfileScreen = ({ bodyMetricsDB, userId, db, APP_ID, logDB, auth, isAdmin, onDeletionStaged }) => {
    ...
    const handleLogout = async () => {
        if (confirm('確定要登出嗎？')) await logoutUser();
    };
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Google 帳號</h3>
                <p className="text-sm text-gray-600 break-all">{user?.email || '未登入'}</p>
                <div className="flex justify-between text-sm mt-5">
                    <button onClick={handleDeleteAccount} disabled={isLoading || user?.uid !== userId || isAdmin} className="text-red-600 disabled:opacity-50">刪除帳號與資料</button>
                    <button onClick={handleLogout} className="text-gray-600">登出</button>
                </div>
            </div>
...
const MenuScreen = ({ setSelectedDailyPlanId, selectedDailyPlanId, plansDB, movementDB, db, userId, APP_ID, setScreen, currentLog, setCurrentLog }) => {
    ...
    // The plan editor keeps local draft fields synchronized with the selected plan.
    useEffect(() => {
...
// ----------------------------------------------------
const AnalysisScreen = ({ logDB, movementDB, db, APP_ID, userId }) => {
...
const App = () => {
    ...
    // Google 登入與 AccessRequests 狀態共同決定產品資料是否可載入。
    useEffect(() => {
        let active = true;
        ...
        return () => { active = false; unsubscribeRequest?.(); unsub(); };
    }, []);
    ...
    useEffect(() => {
        ...
        setupInitialData(db, APP_ID, currentUser).catch(() => {
            // Firestore Rules and the access listener remain the authorization boundary.
        });
...
// 新增：初始化預設動作 (針對新用戶)
const setupInitialData = async (db, APP_ID, userAuth) => {
