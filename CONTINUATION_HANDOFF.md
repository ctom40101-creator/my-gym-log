...
- Run `npm test`, `npm run lint`, `npm run build`, `npm run test:rules`, and `npm exec --yes --package=wrangler@4.137.0 -- wrangler deploy --dry-run --config retention/wrangler.jsonc --outdir retention/.dry-run` afresh before any freeze. The latest local unit result was 76/76 PASS; lint, build, and Worker dry-run PASS. Those results do not replace emulator evidence. Read the branch HEAD fresh; the first WIP checkpoint was `636f118faaa32c33c1581d1f04ac9c4f1e8c579a` and later safety-test edits may advance it.
